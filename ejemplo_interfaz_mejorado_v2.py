...
class AplicacionInterfazMejoradaV2:
    def __init__(self):
        self.root = ctk.CTk()
        self.root.title("Interfaz Moderna Mejorada v2 - Python GUI Demo")
            ...
            with open("datos_usuarios.json", "r", encoding="utf-8") as f:
                ...
                try:
                    ...
                    if isinstance(datos_array, list):
                        # Validar que cada elemento sea un diccionario válido
                        for dato in datos_array:
                            if isinstance(dato, dict) and self.validar_dato(dato):
    ...
    def crear_interfaz(self):
        ...
        titulo = ctk.CTkLabel(
            main_frame, 
            text="Formulario de Registro Mejorado v2", 
    ...
    def mostrar_tabla_datos(self):
        ...
        for i, dato in enumerate(self.datos_guardados, 3):
            try:
                # Verificar que dato sea un diccionario
                if not isinstance(dato, dict):
                    continue
                
                # Obtener valores con manejo de errores
                nombre = str(dato.get('nombre', ''))[:18].ljust(20)
                email = str(dato.get('email', ''))[:23].ljust(25)
                edad = str(dato.get('edad', ''))[:8].ljust(8)
                genero = str(dato.get('genero', ''))[:13].ljust(15)
                
                # Manejar intereses (puede ser lista o string)
                intereses = dato.get('intereses', [])
                ...
                fecha = str(dato.get('fecha_registro', ''))[:16]
                
                linea = f"{nombre}{email}{edad}{genero}{intereses_str}{fecha}\n"
                tabla_text.insert(f"{i}.0", linea)
                
            except Exception as e:
                # Si hay error con un dato, continuar con el siguiente
...
if __name__ == "__main__":
    app = AplicacionInterfazMejoradaV2()
    app.ejecutar() 